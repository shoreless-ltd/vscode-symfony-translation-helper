...
export class TwigParser extends EmbeddedActionsParser {
    ...
    StringLiteral = this.RULE<() => StringLiteral>(
        ...
        () => {
            const token = this.CONSUME(t.StringToken);
            return {
        ...
        () => {
            ...
            this.MANY(() => {
                const element = this.OR([
                    ...
                    {
                        ALT: () => {
                            const token = this.CONSUME(t.StringInterpolationStringPartToken);
                            return {
                                type: NodeKind.StringLiteral,
                                value: token.image,
                                _source: token
                ...
                this.OR([
                    {
                        ALT: () => {
                            ...
                            object = {
                                // TODO: fix circular reference when adding FilterExpression to LeftHandSideExpression sub types
                                // @ts-ignore
                                type: NodeKind.FilterExpression,
                                expression: object,
                                filter: this.SUBRULE(this.Filter),
                            };
                        },
                    },
                    {
    ...
    // @ts-ignore
    UnaryExpression = this.RULE<() => UnaryExpression> (NodeKind.UnaryExpression, () => this.OR([
            {
    ...
    Text = this.RULE<() => Text>(NodeKind.Text, () => {
        const token = this.CONSUME(t.TextToken);
        return {
            type: NodeKind.Text,
            value: token.image,
            _source: token
