...
export const TransToken = createToken(
    {
        name: 'TransToken',
        line_breaks: false,
        pattern: (text, offset, matchedTokens: IToken[]): CustomPatternMatcherReturn | null => {
            if (
                ((matchedTokens.length > 1 && matchedTokens[matchedTokens.length - 1].tokenType.name === 'WhiteSpaceToken' && matchedTokens[matchedTokens.length - 2].tokenType.name === 'LBlockToken')
                || (matchedTokens.length && matchedTokens[matchedTokens.length - 1].tokenType.name === 'LBlockToken'))
                && /^trans\b/.test(text.substring(offset))
            ) {
                return ['trans'];
            }
